...
//
// Vendored from miniflux/v2 internal/reader/sanitizer/srcset.go (Apache-2.0).
// Source: https://github.com/miniflux/v2/blob/main/internal/reader/sanitizer/srcset.go
//
// Wire-local modifications:
//   - Package renamed from `sanitizer` to `extract` (single-package vendor).
//   - Exported `ParseSrcSetAttribute` made package-private (`parseSrcSetAttribute`)
//     since Wire only uses it internally from sanitizer.go.
...
func (c imageCandidates) String() string {
	...
	for _, ic := range c {
		if ic.Descriptor != "" {
			out = append(out, ic.ImageURL+" "+ic.Descriptor)
		} else {
			out = append(out, ic.ImageURL)
		}
	}
	return strings.Join(out, ", ")
}

// parseSrcSetAttribute returns the list of image candidates from the srcset
// attribute. Behavior follows the WebKit HTMLSrcsetParser implementation.
...
func isComma(character byte) bool { return character == ',' }
